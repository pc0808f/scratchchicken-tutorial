...
重複無限次
  如果 <(遊戲狀態) = [1]> 那麼
    建立自己的分身
    變數 [障礙物間隔 v] 設為 (隨機取數(1.5)到(3))  ← 隨機間隔
    等待 (障礙物間隔) 秒
  結束
結束
```
...
#### 5.2 視覺效果：移動的地面（營造奔跑感）

{: .important }
> **💡 重要觀念：善用別人的創作！**
>
> 在程式設計的世界裡，**不是所有東西都要自己從頭做**！
>
> 就像蓋房子時會使用現成的磚塊一樣，
> 程式設計也可以使用別人做好的「角色」或「程式碼」。
>
> **重點是**：你要能**理解**別人的程式在做什麼！
...
我們提供了一個**別人做好的 ground 角色**，
它使用了進階的「分身系統」技巧，可以做出完美的無縫循環地面。
...
**它使用的技術**：
- 🎯 **分身系統**：建立 3 個地面分身並排，形成長長的地面
- 🎯 **變數控制**：用一個變數統一控制所有分身的位置
- 🎯 **廣播訊息**：收到 `game start` 訊息後開始移動

<details>
<summary><strong>💡 進階說明：程式原理（選讀）</strong></summary>
...
2. **統一移動**：
   - 用變數 `ground xpos` 控制所有分身
   - 每次改變 `ground xpos`，就廣播 `ground move`
   - 所有分身收到訊息後，更新自己的位置

3. **無縫循環**：
   - 當 `ground xpos < -1440` 時重置為 0
   - 因為有 3 個分身，所以分身2 會接替分身3 的位置
   - 看起來就像連續不斷的地面！
...
**下載檔案**：
- 📥 [下載 ground 角色圖片](./resources/ground角色/)
- 或從課程資源區下載 `ground.sprite3` 檔案（如果有提供）

**匯入方式**：

**方式 A：如果有 .sprite3 檔案**
1. 在 Scratch 右下角點擊「上傳角色」圖示（📂）
2. 選擇 `ground.sprite3` 檔案
3. 完成！角色和程式都匯入了

**方式 B：手動建立**
1. 新增一個空白角色，命名為 `ground`
2. 匯入 4 個造型：`rect3104.png` ~ `rect3107.png`
3. 複製程式碼（參考提供的程式截圖 `ground程式.png`）
...
當綠旗被點擊
造型換成 [rect3104 v]
變數 [scroll coefficient v] 設為 [-10]    ← 改這裡
```
...
如果地面的高度不對，在 ground 角色中找到：
```scratch
當收到訊息 [game start v]
顯示
如果 <(clone?) = [1]> 那麼
  定位到 x:(360) y:(-32)    ← 改 y 座標
```

建議：如果電子雞在 `y = -80`，地面可以設在 `y = -90` 或 `y = -100`

---

### **方法二：自己動手做簡單版**

如果你想自己理解並製作地面，也可以用簡單的方法：

<details>
<summary><strong>點擊展開：自己做地面的簡單方法</strong></summary>

#### **簡單版原理**

使用**重複圖樣**的地面，移動後重置位置，因為圖樣重複所以看不出來。
...
**思考看看**：
1. 你覺得「使用別人的程式碼」和「抄襲」有什麼不同？
2. 什麼時候應該自己做？什麼時候可以用別人的？
3. 如果你用了別人的角色，應該怎麼做才是尊重創作者？
